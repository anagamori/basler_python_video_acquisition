# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 10:40:22 2022
...
def video_recording(queue,queue2,queue3,enable_prev,video_length,buffer_size,fps,count_video,mouse_ID,session_ID):
        ...
        if j == video_length:    
            ...
            if flag == False and k == 0: # another tirgger never happened 
                ...
                for x in range(k+1):
                    ...
                    queue3_new = deque([])
                    if x == 0:
                        for y in range(int(video_length+buffer_size)):
                            queue_new.append(queue[y])
                            queue2_new.append(queue2[y])
                            queue3_new.append(queue3[y])
                            
                        idx = int(video_length+buffer_size)                      
                    else:
                        for y in range(idx,idx+int(video_length)-(int(video_length)-int(idx_trigger[x-1]))):
                            queue_new.append(queue[y])
                            queue2_new.append(queue2[y])
                            queue3_new.append(queue3[y])
                ...
                j = video_length-idx_trigger[k-1] # reset counter to the number of missing images in the buffer for the next video  
                flag = False                             
                
                
        enable_prev = enable
        
    return nFrame,queue,queue2,queue3,count_video
